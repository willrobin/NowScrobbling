...
function nowscrobbling_render_shortcode_callback() {
    ...
    if ( ! isset( $_POST['_wpnonce'] ) || ! wp_verify_nonce( $_POST['_wpnonce'], 'nowscrobbling_nonce' ) ) {
...
function nowscrobbling_clear_cache_callback() {
    ...
    // Nonce check
    if (!isset($_POST['_wpnonce']) || !wp_verify_nonce($_POST['_wpnonce'], 'nowscrobbling_admin_nonce')) {
        wp_send_json_error(['message' => 'Ungültige Anfrage (nonce).'], 403);
    }
